...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Staging_DIM_PATCH]') AND type IN (N'U'))
    DROP TABLE [dbo].[Staging_DIM_PATCH]
GO
...
FROM OPENQUERY([NABLESNOWFLAKE], '
    SELECT 
        ...
        -- Always convert to VARCHAR in Snowflake
CAST(PUBLISH_DATE AS VARCHAR(8000)) AS PUBLISH_DATE,
	--	TO_VARCHAR(CASE 
    --        WHEN PUBLISH_DATE IS NULL THEN ''1990-01-01 00:00:00''
    --        ELSE CAST(PUBLISH_DATE AS VARCHAR(8000))
    --    END) AS PUBLISH_DATE,
...
CAST(DSS_UPDATED AS VARCHAR(8000)) AS DSS_UPDATED
    FROM SHARED_ANALYTICS_DATA.SHARED_DATA.DIM_PATCH
...
-- Merge statement
MERGE [dbo].[DIM_PATCH] AS TARGET
USING [dbo].[Staging_DIM_PATCH] AS SOURCE
