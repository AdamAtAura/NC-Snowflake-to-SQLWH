...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Staging_FACT_UTILIZATION_DAY]') AND type in (N'U'))
    DROP TABLE [dbo].[Staging_FACT_UTILIZATION_DAY]
GO

SELECT * 
INTO [dbo].[Staging_FACT_UTILIZATION_DAY]
FROM OPENQUERY([NABLESNOWFLAKE], '
    SELECT
       ...
       ,DATE_TRUNC(''HOUR'', SCAN_DATE) AS SCAN_DATE
	   ,UTILIZATION_PROFILE_DISK_DK
    ...
    FROM SHARED_ANALYTICS_DATA.SHARED_DATA.FACT_UTILIZATION_DAY
    WHERE (DSS_CREATED >= DATEADD(DAY, -5, CURRENT_TIMESTAMP()) AND DSS_CREATED <= DATEADD(DAY, 0, CURRENT_TIMESTAMP()))
       OR (DSS_UPDATED >= DATEADD(DAY, -5, CURRENT_TIMESTAMP()) AND DSS_UPDATED <= DATEADD(DAY, 0, CURRENT_TIMESTAMP()))
')

MERGE [dbo].[FACT_UTILIZATION_DAY] AS TARGET
USING [dbo].[Staging_FACT_UTILIZATION_DAY] AS SOURCE
...
AND TARGET.SCAN_DATE = SOURCE.SCAN_DATE
AND TARGET.UTILIZATION_PROFILE_DISK_DK = SOURCE.UTILIZATION_PROFILE_DISK_DK
...
WHEN MATCHED AND (
    TARGET.SCAN_DATE <> SOURCE.SCAN_DATE OR
    CAST(SUBSTRING(TARGET.PROFILE_NAME,1,8000) AS CHAR(8000)) <> CAST(SUBSTRING(SOURCE.PROFILE_NAME,1,8000) AS CHAR(8000)) OR
	TARGET.UTILIZATION_PROFILE_DISK_DK <> SOURCE.UTILIZATION_PROFILE_DISK_DK OR
    TARGET.SERVICE_INSTANCE_DK <> SOURCE.SERVICE_INSTANCE_DK OR
...
THEN UPDATE SET
    TARGET.SCAN_DATE = SOURCE.SCAN_DATE,
	TARGET.UTILIZATION_PROFILE_DISK_DK = SOURCE.UTILIZATION_PROFILE_DISK_DK,
    TARGET.PROFILE_NAME = CAST(SUBSTRING(SOURCE.PROFILE_NAME,1,8000) AS CHAR(8000)),
    TARGET.SERVICE_INSTANCE_DK = SOURCE.SERVICE_INSTANCE_DK,
	...
	WHEN NOT MATCHED BY TARGET THEN
		INSERT (
			[TENANT_DK],[DEVICE_DK],[DEVICE_SK],[SITE_SK],[SITE_DK],[CUSTOMER_SK],[CUSTOMER_DK],[SCAN_DATE_DK]
           ,[SCAN_DATE],[UTILIZATION_PROFILE_DISK_DK],[PROFILE_NAME],[SERVICE_INSTANCE_DK],[MAX_UNITS_AVAILABLE]
		...
		VALUES (
			...
			SOURCE.CUSTOMER_SK, SOURCE.CUSTOMER_DK, SOURCE.SCAN_DATE_DK,
			SOURCE.SCAN_DATE, SOURCE.UTILIZATION_PROFILE_DISK_DK, CAST(SUBSTRING(SOURCE.PROFILE_NAME,1,8000) AS CHAR(8000)), SOURCE.SERVICE_INSTANCE_DK, SOURCE.MAX_UNITS_AVAILABLE, SOURCE.MIN_UNITS_AVAILABLE,
			SOURCE.SUM_UNITS_AVAILABLE, SOURCE.AVG_UNITS_AVAILABLE, SOURCE.MAX_UNITS_CONSUMED, SOURCE.MIN_UNITS_CONSUMED, 
			SOURCE.SUM_UNITS_CONSUMED, SOURCE.AVG_UNITS_CONSUMED, SOURCE.MAX_UNITS_TOTAL, SOURCE.MIN_UNITS_TOTAL, 
			SOURCE.SUM_UNITS_TOTAL, SOURCE.AVG_UNITS_TOTAL, SOURCE.MAX_USAGE_PERCENT, SOURCE.MIN_USAGE_PERCENT, 
			SOURCE.SUM_USAGE_PERCENT, SOURCE.AVG_USAGE_PERCENT, SOURCE.ROW_COUNT, SOURCE.DSS_CREATED, SOURCE.DSS_UPDATED
			);
