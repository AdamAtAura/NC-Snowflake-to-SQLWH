...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[DIM_SITE]') AND type in (N'U'))
DROP TABLE [dbo].DIM_SITE
GO

SELECT * 
INTO DIM_SITE
FROM OPENQUERY([NABLESNOWFLAKE], 'SELECT SITE_SK
	  ,TENANT_DK
	  ,SITE_DK
	  ,CUSTOMER_DK
	  ...
	  ,DSS_CREATED
	  ,DSS_UPDATED
	FROM SHARED_ANALYTICS_DATA.SHARED_DATA.DIM_SITE')
